...
export default {
	...
	setup(props, context) {
		...
		const submitCode = () => {
			Log.info("pin: " + JSON.stringify(collectPin()));
			submitLoading.value = true;

			if (collectPin().pin.length < 6) {
				submitLoading.value = false;
				errorMessage.value = "All fields must be filled";
			} else {
				userPin.value = collectPin().pin;

				context.emit("success", userPin.value);
				Log.info(collectPin().pin);
				close();
				resetInput();
			}
		};
