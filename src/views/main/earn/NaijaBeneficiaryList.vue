<template>
	...
	<div class="">
		<div style="border-bottom: 0.8px solid #f1f1f1" class="pb-4 px-6 flex">
			<div @click="$emit('close')" class="flex mr-3 items-center">
				<svg
					width="24"
					height="19"
					viewBox="0 0 24 19"
		...
		<div class="flex flex-col px-6 pt-10">
			<div v-for="beneficiary in beneficiaryList" :key="beneficiary.name" class="flex mb-2">
				...
				<div class="flex flex-col">
					<span class="blacktext fs-14 fw-500">{{ beneficiary.name }}</span>
					<span class="fw-400 fs-14 tx-666666"
						>{{ beneficiary.accountNo }} - {{ beneficiary.bank }}</span
					>
				</div>
			</div>
		</div>
